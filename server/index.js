...
app.post('/drinkmenu', (req, res) => {
  ...
  pool
    .query(
      `SELECT * FROM drinks WHERE drink_name = '${drinkName}' AND place_id = '${req.body.place_id}'`
    )
    .then((data) => {
      if (data.rows.length > 0) {
        ...
        pool
          .query(
            `INSERT INTO drinks (drink_name, drink_rating, place_id) VALUES ('${drinkName}', ${rating}, '${req.body.place_id}')`
          )
          .then((x) => {
...
app.post('/drinkrating', (req, res) => {
  if (req.body.rating === '1') {
    pool
      .query(
        `UPDATE drinks SET drink_rating = drink_rating + 1 WHERE id = ${Number(
          req.body.drink_id
        )}`
      )
      .then((x) => {
...
app.get('/drinkmenu', (req, res) => {
  pool
    .query(`SELECT * FROM drinks WHERE place_id = '${req.query.place_id}'`)
    .then((data) => {
...
app.post('/shopratings', (req, res) => {
  pool
    .query(
      `SELECT shop_rating FROM shops WHERE place_id = '${req.body.place_id}'`
    )
    .then((data) => {
      if (data.rows.length > 0) {
        if (req.body.rating === '1') {
          pool
            .query(
              `UPDATE shops SET shop_rating = shop_rating + 1 WHERE place_id = '${req.body.place_id}'`
            )
            .then((x) => {
        ...
        if (req.body.rating === '1') {
          pool
            .query(
              `INSERT INTO shops (place_id, shop_rating) VALUES ('${req.body.place_id}', 1)`
            )
            .then((x) => {
          ...
          pool
            .query(
              `INSERT INTO shops (place_id, shop_rating) VALUES ('${req.body.place_id}', 0)`
            )
            .then((x) => {
              res.status(200).send('Shop rating added!');
            })
...
app.get('/shopratings', (req, res) => {
  let queryArg = req.body.shops
    .replace(/, /g, "', '")
    .replace('[', "('")
    .replace(']', "')");

  pool
    .query(`SELECT * FROM shops WHERE place_id IN ${queryArg}`)
    .then((data) => {
      res.status(200).send(data.rows);
    })
    .catch((err) => {
...
app.post('/favorites', (req, res) => {
  if (req.body.isCoffee === 'true') {
    pool
      .query(
        `SELECT * FROM favorites WHERE user_id = ${Number(
          req.body.user_id
        )} AND drink_id = ${Number(req.body.drink_id)} AND place_id = '${
          req.body.place_id
        }'`
      )
      .then((data) => {
        if (data.rows.length > 0) {
          res.status(400).send('This drink is already a favorite of yours!');
        } else {
          pool
            .query(
              `INSERT INTO favorites (isCoffee, user_id, place_id, drink_id) VALUES (true, ${Number(
                req.body.user_id
              )}, '${req.body.place_id}', ${Number(req.body.drink_id)})`
            )
            .then((x) => {
              res.status(200).send('Drink added to favorites!');
            })
            .catch((err) => {
              console.error(err);
              res.status(500).send();
    ...
    pool
      .query(
        `SELECT * FROM favorites WHERE user_id = ${Number(
          req.body.user_id
        )} AND place_id = '${req.body.place_id}'`
      )
      .then((data) => {
        if (data.rows.length > 0) {
          res.status(400).send('This shop is already a favorite of yours!');
        } else {
          pool
            .query(
              `INSERT INTO favorites (isCoffee, user_id, place_id, drink_id) VALUES (false, ${Number(
                req.body.user_id
              )}, '${req.body.place_id}', null)`
            )
            .then((x) => {
              res.status(200).send('Shop added to favorites!');
            })
            .catch((err) => {
              console.error(err);
              res.status(500).send();
...
app.get('/userfavorites', (req, res) => {
  ...
  pool
    .query(
      `SELECT * FROM favorites WHERE user_id = ${Number(req.body.user_id)}`
    )
    .then((data) => {
      for (let favorite of data.rows) {
        if (favorite['iscoffee']) {
          returnObject.favoriteDrinks.push(favorite);
        } else {
